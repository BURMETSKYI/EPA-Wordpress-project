...
if mysql -h "$rds_edpoint" -u "$db_username" -p"$db_password" -e "SELECT 1;" > /dev/null 2>&1; then
...
if mysql -h "$rds_edpoint" -u "$db_username" -p"$db_password" -e "USE admin;" > /dev/null 2>&1; then
...
WP_TABLES=$(mysql -h "$rds_edpoint" -u "$db_username" -p"$db_password" -e "USE admin; SHOW TABLES LIKE 'wp_%';" 2>/dev/null)
if [ ! -z "$WP_TABLES" ]; then
...
if mysql -h "$rds_edpoint" -u "$db_username" -p"$db_password" -e "USE admin; SELECT option_name FROM wp_options WHERE option_name IN ('siteurl', 'home') LIMIT 1;" > /dev/null 2>&1; then
